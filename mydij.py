...
def convert_adjacency_matrix_to_dict(adj_matrix):
    ...
    for i in range(size):
        graph_dict[i+1] = {}
        for j in range(size):
            if adj_matrix[i, j] != 0:  # There is an edge
                graph_dict[i+1][j+1] = int(adj_matrix[i, j])
    return graph_dict

# Dictionary to hold the graphs
graphs = {}
...
def shortest_path(graph_name):
    ...
    # Load the graph if it's not already loaded
    if graph_name not in graphs:
        graph_file_path = f'{graph_name}.mat'  # Assuming the file is named after the graph
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=80, debug=False)  # Set debug=False for production use
